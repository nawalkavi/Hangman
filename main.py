...
auDictionary = {  # Dictionary storing all the frames of each individual Among Us colour.
	# "purple": {
	# 	1: pygame.transform.scale(pygame.image.load("Assets/Sprites/Purple/Frame 1.png"), (60, 70)).convert_alpha(),  # convert_alpha() converts the image file type into something Python can work with easier, which helps to optimise the game a bit more.
...
# Positions.
menuText_xPos = 600  # Menu.
menuText_yPos = 200

ground_xPos = 600  # Ground.
ground_yPos = 400
...
blueAU_xPos = -190  # Blue.
blueAU_yPos = 705

orangeAU_xPos = 600  # Orange.
orangeAU_yPos = 400
				...
				if event.key == pygame.K_w:
					for imageName in auDictionary:  # Loops through each colour in the dictionary.
						...
						if image["currentFrame"] < len(image) - 1:  # Length of each colour dictionary is 5, which is why it is len(image) - 1.
							...
							image["currentFrame"] = 1  # If the current index is not less than 4 (the last frame), then it is sent back to 1 (the first frame).
